...
The inference results will be saved in the folder `inference_output_latest`. This folder will contain four files: 
...
python $atacworks/scripts/peaksummary.py \
    ...
    --prefix inference_output_latest/NK_inferred.peak_calls \
    --out_dir inference_output_latest \
    --minlen 20
```
This produces a file `inference_output_latest/NK_inferred.peak_calls.bed` with 8 columns:
1. chromosome
...
```
macs2 bdgpeakcall -i inference_output_latest/inferred.peaks.bedGraph -o inference_output_latest/inferred.peaks.narrowPeak -c 0.5
```
