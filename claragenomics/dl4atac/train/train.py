...
def train(*, rank, gpu, task, model, train_loader, loss_func, optimizer, 
    ...
    epoch_formatter = "Epoch " + equal_width_formatter(total=epochs).format(epoch)
    start = time.time()
    forward_time = 0.
    backward_time = 0.
    print_time = 0.
    ...
    for i, batch in enumerate(train_loader):
        ...
        if task == 'regression' or task == 'classification':
            ...
            total_loss_value_reg, losses_values_reg = loss_func[0](pred[0], y[0])
            total_loss_value_cla, losses_values_cla = loss_func[1](pred[1], y[1])
    ...
    myprint(epoch_formatter + " Time Taken: {:7.3f}s".format(time.time()-start), color='yellow', rank=rank)
